...
from .agent_types import ShortTermMemory
    ...
    def get_action(self, max_retries=3):
        ...
        while retries < max_retries:
            try:
                ...
                if self.short_term_memory.memory:
                    prompt_str = prompt_str.replace("{conversation_history}", str(self.short_term_memory.memory[-1]))
                else:
                    prompt_str = prompt_str.replace("{conversation_history}", "No previous conversation history.")            
                response = chain.invoke({
                    "goal": self.short_term_memory.goal,
                    "conversation_history": self.short_term_memory.memory[-1] if self.short_term_memory.memory else "",
                })
