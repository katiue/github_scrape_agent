...
import { Send, AlertTriangle, ChevronDown, ChevronRight, Bot } from "lucide-react"
...
export function ChatInterface({ onResponse, currentRepo, currentFiles, onMessagesChange }: ChatInterfaceProps) {
  ...
  const handleSendMessage = async () => {
    ...
    setMessages((prev) => [...prev, { id: messageId, role: "user", content: query }])

    // Immediately add a placeholder assistant message that will be updated
    setMessages((prev) => [...prev, { id: responseId, role: "assistant", content: "", thinking: "" }])
    ...
    try {
        ...
        for (const event of events) {
          if (event.startsWith("data: ")) {
            try {
              ...
              setMessages((prev) =>
                prev.map((msg) =>
                  msg.id === responseId ? { ...msg, content: responseContent, thinking: thinkingContent } : msg,
                ),
              )
      ...
      if (error instanceof Error && error.name === "AbortError") {
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === responseId ? { ...msg, content: "Sorry, an error occurred while generating a response." } : msg,
          ),
        )
      }
    } finally {
  ...
  return (
    <div className="flex flex-col h-full">
      {messages.length > 0 ? (
              ...
              {messages.map((message) => (
                ...
                >
                  ...
                  <div className="whitespace-pre-wrap">
                    {message.content ||
                      (message.role === "assistant" && isLoading && <div className="animate-pulse">Thinking...</div>)}
                  </div>
                </div>
              ))}
        ...
        // Welcome state UI when no messages
        <div className="flex flex-col items-center justify-center h-full p-4">
          <div className="flex flex-col items-center max-w-md w-full space-y-8">
            ...
            <div className="text-center space-y-2">
              <h1 className="text-2xl font-bold">GitHub AI Assistant</h1>
              <p className="text-muted-foreground">
                Ask me about GitHub repositories, code analysis, or any programming questions.
              </p>
            </div>
            ...
            <div className="text-xs text-muted-foreground text-center">
              Try asking about a GitHub repository or code analysis
            </div>
          </div>
        </div>
      )}
